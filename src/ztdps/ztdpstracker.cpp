...
CzTDPSTracker::CzTDPSTracker(std::string strWalletFile)
{
...
CzTDPSTracker::~CzTDPSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzTDPSTracker::Init()
{
...
bool CzTDPSTracker::Archive(CMintMeta& meta)
{
...
bool CzTDPSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzTDPSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzTDPSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzTDPSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzTDPSTracker::GetSpendCache(const uint256& hashStake)
{
...
std::vector<uint256> CzTDPSTracker::GetSerialHashes()
{
...
CAmount CzTDPSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzTDPSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

//Does a mint in the tracker have this txid
bool CzTDPSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzTDPSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzTDPSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzTDPSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzTDPSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzTDPSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzTDPSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzTDPSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzTDPSWallet* zTDPSWallet)
{
    bool iszTDPSWalletInitialized = (NULL != zTDPSWallet);
    CMintMeta meta;
    ...
    if (! iszTDPSWalletInitialized)
        zTDPSWallet = new CzTDPSWallet(strWalletFile);
    meta.isSeedCorrect = zTDPSWallet->CheckSeed(dMint);
    if (! iszTDPSWalletInitialized)
        delete zTDPSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzTDPSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTDPSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzTDPSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzTDPSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzTDPSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzTDPSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzTDPSWallet* zTDPSWallet = new CzTDPSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zTDPSWallet);
        }
        delete zTDPSWallet;
        LogPrint("zero", "%s: added %d dztdps from DB\n", __func__, listDeterministicDB.size());
    }
