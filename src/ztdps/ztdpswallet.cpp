...
CzTDPSWallet::CzTDPSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing ztdps seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTDPSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTDPSSeed_deprecated()) {
                    LogPrintf("%s: Updated zTDPS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old ztdps seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of ztdps wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzTDPSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTDPSCount(nCountLastUsed);
    else if (!walletdb.ReadZTDPSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTDPSWallet::Lock()
{
    seedMaster = 0;
}

void CzTDPSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTDPSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTDPS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTDPSWallet::LoadMintPoolFromDB()
{
...
void CzTDPSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTDPSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTDPSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->ztdpsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzTDPSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTDPS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to ztdpsTracker which also adds to database
    pwalletMain->ztdpsTracker->Add(dMint, true);
...
void CzTDPSWallet::SeedToZTDPS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzTDPSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTDPSWallet::GenerateDeterministicZTDPS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTDPSWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTDPSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzTDPSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
