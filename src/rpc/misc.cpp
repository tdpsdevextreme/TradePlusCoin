...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total tradeplus coin balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in tradeplus_coin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in tradeplus_coin/kb\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"tradeplus_coinaddress\"\n"
            "\nReturn information about the given tradeplus_coin address.\n"

            "\nArguments:\n"
            "1. \"tradeplus_coinaddress\"     (string, required) The tradeplus_coin address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"tradeplus_coinaddress\", (string) The tradeplus_coin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: TradePlus_Coin address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are tradeplus_coin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) tradeplus_coin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"tradeplus_coinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"tradeplus_coinaddress\"  (string, required) The tradeplus_coin address to use for the signature.\n"
