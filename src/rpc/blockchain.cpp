...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue ztdpsObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ztdpsObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    ztdpsObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zTDPSsupply", ztdpsObj));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZTDPS()) ? "Not available" : std::to_string(
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"zTDPSsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zTDPS denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zTDPS denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zTDPS denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zTDPS denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zTDPS denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zTDPS denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zTDPS denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zTDPS denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zTDPS denominations\n"
            "  }\n"
            "}\n"
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in tradeplus coin\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of tradeplus coin addresses\n"
            "     \"tradeplus coinaddress\"   	 	(string) tradeplus_coin address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (txin.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (isPublicSpend) {
                        ...
                        if (!ZTDPSModule::parseCoinSpend(txin, tx, prevOut, publicSpend)) {
...
UniValue getblockindexstats(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
                ...
                "  }\n"
                "  \"txbytes\": xxxxx                (numeric) Sum of the size of all txes (zTDPS excluded) over block range\n"
                "  \"ttlfee\": xxxxx                 (numeric) Sum of the fee amount of all txes (zTDPS mints excluded) over block range\n"
                "  \"ttlfee_all\": xxxxx             (numeric) Sum of the fee amount of all txes (zTDPS mints included) over block range\n"
