...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that ztdpsspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 TDPS at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTDPS send
enum ZerocoinSpendStatus {
    ZTDPS_SPEND_OKAY = 0,                            // No error
    ZTDPS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTDPS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTDPS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTDPS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTDPS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTDPS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTDPS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTDPS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTDPS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTDPS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTDPS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTDPS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTDPS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTDPS_SPENT_USED_ZTDPS = 14,                      // Coin has already been spend
    ZTDPS_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZTDPS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    static const int STAKE_SPLIT_THRESHOLD = 4;
    ...
    std::string GetUniqueWalletBackupName(bool fztdpsAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzTDPSTracker> ztdpsTracker;
    ...
    void setZWallet(CzTDPSWallet* zwallet)
    {
        zwalletMain = zwallet;
        ztdpsTracker = std::unique_ptr<CzTDPSTracker>(new CzTDPSTracker(strWalletFile));
    }

    CzTDPSWallet* getZWallet() { return zwalletMain; }
    ...
    /** zTDPS reset */
    boost::signals2::signal<void()> NotifyzTDPSReset;
