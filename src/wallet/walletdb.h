...
public:
    ...
    bool WriteZTDPSSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZTDPSSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZTDPSSeed_deprecated(uint256& seed);
    bool EraseZTDPSSeed();
    bool EraseZTDPSSeed_deprecated();

    bool WriteZTDPSCount(const uint32_t& nCount);
    bool ReadZTDPSCount(uint32_t& nCount);
