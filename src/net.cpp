...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (TradePlus_Coin users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        std::string strDesc = "TradePlus Coin " + FormatFullVersion();
...
void ThreadOpenAddedConnections()
{
    {
        LOCK(cs_vAddedNodes);
		mapMultiArgs["-addnode"].push_back("157.245.164.19");
		mapMultiArgs["-addnode"].push_back("167.71.117.89");
		mapMultiArgs["-addnode"].push_back("167.71.117.112");
		mapMultiArgs["-addnode"].push_back("167.71.125.170");
		mapMultiArgs["-addnode"].push_back("167.71.115.18");
		mapMultiArgs["-addnode"].push_back("167.71.126.74");
		mapMultiArgs["-addnode"].push_back("157.245.173.128");
...
bool BindListenPort(const CService& addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. TradePlus Coin is probably already running."), addrBind.ToString());
        else
