                                                ...
                                                fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zTDPS ought to be enough for anybody." - Bill Gates, 2017
    ui->zTDPSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTDPSSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_pushButtonSpendzTDPS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTDPS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zTDPS, true));
        if (!ctx.isValid()) {
...
void PrivacyDialog::on_pushButtonZTdpsControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZTdpsControlDialog* zTdpsControl = new ZTdpsControlDialog(this);
    zTdpsControl->setModel(walletModel);
    zTdpsControl->exec();
}

void PrivacyDialog::setZTdpsControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTdpsSelected_int->setText(QString::number(nAmount));
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTDPS </b>";
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zTDPS </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zTDPS </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzTDPSSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzTDPS->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zTDPS
        //ui->pushButtonMintzTDPS->setEnabled(false);
        //ui->pushButtonMintzTDPS->setToolTip(tr("zTDPS is currently disabled due to maintenance."));

        // Spend zTDPS
        ui->pushButtonSpendzTDPS->setEnabled(false);
        ui->pushButtonSpendzTDPS->setToolTip(tr("zTDPS is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zTDPS
        //ui->pushButtonMintzTDPS->setEnabled(true);
        //ui->pushButtonMintzTDPS->setToolTip(tr("PrivacyDialog", "Enter an amount of TDPS to convert to zTDPS", 0));

        // Spend zTDPS
        ui->pushButtonSpendzTDPS->setEnabled(true);
        ui->pushButtonSpendzTDPS->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
