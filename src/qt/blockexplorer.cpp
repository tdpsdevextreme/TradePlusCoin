...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::TDPS, nValue);
...
{
    std::string genesisblockhash = "000002dfd56f68eff2a54608574f41c403c4a4e75648e1aeb1c58a9c60946f63";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (tradeplus_coin.conf).");
            QMessageBox::warning(this, "TradePlus Coin Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
