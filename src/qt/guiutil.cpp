...
#define URI_SCHEME "tradeplus_coin"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a TradePlus Coin address (e.g. %1)").arg("T1vRy83SQgA1zrW16hjcHJtcfip5krtqfU"));
...
{
    // return if URI is not valid or is no TradePlus_Coin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::TDPS, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert tradeplus_coin:// to tradeplus_coin:
    //
    //    Cannot handle this later, because tradeplus_coin:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::TDPS, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openConfigfile()
{
    ...
    /* Open tradeplus_coin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TradePlus_Coin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for TradePlus_Coin.lnk
...
{
    return GetAutostartDir() / "tradeplus_coin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a tradeplus_coin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=TradePlus_Coin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the tradeplus_coin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add tradeplus_coin app to startup item list
