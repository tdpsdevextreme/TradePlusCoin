...
#include "qt/tradeplus_coin/send.h"
#include "qt/tradeplus_coin/forms/ui_send.h"
#include "qt/tradeplus_coin/addnewcontactdialog.h"
#include "qt/tradeplus_coin/qtutils.h"
#include "qt/tradeplus_coin/sendchangeaddressdialog.h"
#include "qt/tradeplus_coin/optionbutton.h"
...
SendWidget::SendWidget(TradePlus_CoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::send),
	...
	btnContacts(new QPushButton())
{
	...
	/* Subtitle */
	ui->labelSubtitle1->setText(tr("You can transfer public coins (TDPS)"));
	setCssProperty(ui->labelSubtitle1, "text-subtitle");

	/* Address */
	ui->labelSubtitleAddress->setText(tr("Enter a TradePlus Coin address or contact label"));
	...
	ui->labelAmountSend->setText("0.00 TDPS");
	setCssProperty(ui->labelAmountSend, "text-body1");

	//send button
	ui->pushButtonSave->setText(tr("Send TDPS"));
	...
	setCssProperty(coinIcon, "coin-icon-tdps");
...
void SendWidget::onSendClicked() {
	...
	// will call relock
	if (!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Send_TDPS, true)) {
...
void SendWidget::onCoinControlClicked() {
	if (walletModel->getBalance() > 0) {
		...
		inform(tr("You don't have any TDPS to select."));
	}
}
...
void SendWidget::onContactMultiClicked() {
	if (focusedEntry) {
		...
		CBitcoinAddress tdpsAdd = CBitcoinAddress(address.toStdString());
		if (walletModel->isMine(tdpsAdd)) {
		...
		if (dialog->res) {
			...
			if (walletModel->updateAddressBookLabels(tdpsAdd.Get(), dialog->getLabel().toStdString(), "send")) {
