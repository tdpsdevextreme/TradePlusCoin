...
#include "qt/tradeplus_coin/dashboardwidget.h"
#include "qt/tradeplus_coin/forms/ui_dashboardwidget.h"
#include "qt/tradeplus_coin/sendconfirmdialog.h"
#include "qt/tradeplus_coin/txrow.h"
...
DashboardWidget::DashboardWidget(TradePlus_CoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	// Title
	ui->labelTitle2->setText(tr("Received Rewards"));
	setCssTitleScreen(ui->labelTitle);
	setCssTitleScreen(ui->labelTitle2);

	// Chart labels
	ui->labelChart->setText(tr("Reward statistics"));
	ui->labelTdps->setText(tr("Stake"));
	ui->labelzTdps->setText(tr("Masternode"));
	...
	// Staking Information
	ui->labelMessage->setText(tr("Amount of received rewards."));
	setCssSubtitleScreen(ui->labelMessage);
	setCssProperty(ui->labelSquareTdps, "square-chart-tdps");
	setCssProperty(ui->labelSquarezTdps, "square-chart-ztdps");
	setCssProperty(ui->labelTdps, "text-chart-tdps");
	setCssProperty(ui->labelzTdps, "text-chart-ztdps");
	...
	ui->labelAmountzTdps->setText("0 Masternode");
	ui->labelAmountTdps->setText("0 Stake");
	setCssProperty(ui->labelAmountTdps, "text-stake-tdps-disable");
	setCssProperty(ui->labelAmountzTdps, "text-stake-ztdps-disable");
	...
	ui->btnHowTo->setText(tr("How to get TDPS"));
	...
	if (hasCharts) {
		ui->labelEmptyChart->setText(tr("You have no staking or masternode rewards"));
	}
	else {
...
void DashboardWidget::setStartDay() {
	if (QDate::currentDate().month() == monthFilter) {
		int nday = QDate::currentDate().day();
		if (nday > 9) dayStart = nday - 8;
	}
	else {
		dayStart = 1;
	}
...
void DashboardWidget::loadChart() {
	if (hasStakes) {
		if (!chart) {
			...
			connect(ui->pushButtonChartArrow, SIGNAL(clicked()), this, SLOT(onChartArrowClicked(true)));
			connect(ui->pushButtonChartRight, SIGNAL(clicked()), this, SLOT(onChartArrowClicked(false)));
		}
		refreshChart();
		changeChartColors();
	}
...
void DashboardWidget::changeChartColors() {
	...
	if (isLightTheme()) {
		...
		gridLineColorX = QColor(12, 17, 24);
		linePenColorY = gridLineColorX;
		backgroundColor = linePenColorY;
		axisX->setLabelsBrush(QBrush(QColor("#777")));
		axisY->setLabelsBrush(QBrush(QColor("#777")));
	}
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData->amountsByCache.contains(num)) {
			...
			tdps = (pair.first != 0) ? pair.first / 100000000 : 0;
			ztdps = (pair.second != 0) ? pair.second / 100000000 : 0;

			chartData->totalTdps += pair.first;
			chartData->totalzTdps += pair.second;

		}

		chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesTdps.append(tdps);
		chartData->valueszTdps.append(ztdps);


		int max = std::max(tdps, ztdps);
...
void DashboardWidget::onChartMonthChanged(const QString& monthStr) {
	if (isChartInitialized) {
		int newMonth = ui->comboBoxMonths->currentIndex() + 1;
...
void DashboardWidget::onChartRefreshed() {
	...
	set1 = new QBarSet("Masternode");
	if (isLightTheme()) {
		set0->setColor(QColor("#d55648")); //92,75,125
		set1->setColor(QColor("#fe978b")); //176,136,255
	}
	else {
		set0->setColor(QColor("#d55648")); //92,75,125
		set1->setColor(QColor("#fe978b")); //176,136,255
	...
	set0->append(chartData->valuesTdps);
	set1->append(chartData->valueszTdps);


	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalTdps > 0 || chartData->totalzTdps > 0) {
		setCssProperty(ui->labelAmountTdps, "text-stake-tdps");
		setCssProperty(ui->labelAmountzTdps, "text-stake-ztdps");
	}
	else {
		setCssProperty(ui->labelAmountTdps, "text-stake-tdps-disable");
		setCssProperty(ui->labelAmountzTdps, "text-stake-ztdps-disable");
	}
	forceUpdateStyle({ ui->labelAmountTdps, ui->labelAmountzTdps });
	ui->labelAmountTdps->setText(GUIUtil::formatBalance(chartData->totalTdps, 0).replace(QRegExp("TDPS"), "Stake"));
	ui->labelAmountzTdps->setText(GUIUtil::formatBalance(chartData->totalzTdps, 0).replace(QRegExp("TDPS"), "Masternode"));
	...
	axisY->setRange(0, ((chartData->maxValue > 0) ? chartData->maxValue : 1));
