...
#include "qt/tradeplus_coin/topbar.h"
#include "qt/tradeplus_coin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/tradeplus_coin/lockunlock.h"
#include "qt/tradeplus_coin/qtutils.h"
#include "qt/tradeplus_coin/receivedialog.h"
...
TopBar::TopBar(TradePlus_CoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopTdps}, "amount-small-topbar");
    setCssProperty({ui->labelAmountTdps}, "amount-topbar");
    setCssProperty({ui->labelPendingTdps, ui->labelImmatureTdps}, "amount-small-topbar");
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("TradePlus_Coin")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // Set
    QString totalTdps = GUIUtil::formatBalance(tdpsAvailableBalance, nDisplayUnit);
    // Top
    ui->labelAmountTopTdps->setText(totalTdps);
    

    // Expanded
    ui->labelAmountTdps->setText(totalTdps);
    

    ui->labelPendingTdps->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));

    ui->labelImmatureTdps->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
}
