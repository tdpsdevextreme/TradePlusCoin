...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::TDPS);
    ...
    if (settings.contains("nAnonymizeTradeplus_CoinAmount") || reset)
        SoftSetArg("-anonymizetradeplus_coinamount", settings.value("nAnonymizeTradeplus_CoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in tradeplus_coin.cpp during shotdown to also remove the window positions
