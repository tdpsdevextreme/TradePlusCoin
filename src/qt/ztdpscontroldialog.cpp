...
#include "ui_ztdpscontroldialog.h"
...
std::set<std::string> ZTdpsControlDialog::setSelectedMints;
std::set<CMintMeta> ZTdpsControlDialog::setMints;

bool CZTdpsControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZTdpsControlDialog::COLUMN_DENOMINATION || column == ZTdpsControlDialog::COLUMN_VERSION || column == ZTdpsControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZTdpsControlDialog::ZTdpsControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZTdpsControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zTDPS Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZTdps->setProperty("cssClass", "text-main-purple");
    ui->labelZTdps_int->setProperty("cssClass", "text-main-purple");
...
ZTdpsControlDialog::~ZTdpsControlDialog()
{
    delete ui;
}

void ZTdpsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTdpsControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZTdpsControlWidgetItem* itemDenom(new CZTdpsControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZTdpsControlWidgetItem *itemMint = new CZTdpsControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->ztdpsTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->ztdpsTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zTDPS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zTDPS seed used to mint this zTDPS is not the same as currently hold in the wallet";
            else
...
void ZTdpsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTdpsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZTdps_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZTdpsControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZTdpsControlDialog::GetSelectedMints()
{
...
void ZTdpsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
