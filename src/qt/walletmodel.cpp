...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered tradeplus_coin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 TDPS.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal tradeplus_coin:URI (tradeplus_coin:XyZ...?message=example)
            {
...
static void NotifyzTDPSReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzTDPSReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzTDPSReset.connect(boost::bind(NotifyzTDPSReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzTDPSReset.disconnect(boost::bind(NotifyzTDPSReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->ztdpsTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
