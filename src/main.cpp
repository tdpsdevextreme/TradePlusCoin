...
#include "ztdps/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
#if defined(NDEBUG)
#error "TradePlus_Coin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in utdps) are considered zero fee (for relaying and mining)
...

    {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zTDPS spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!isPrivZerocoinSpend && !isPublicSpend) {
                    return state.Invalid(error("%s: AcceptToMemoryPool failed for tx %s, every input must be a zcspend or zcpublicspend", __func__,
                                        tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-ztdps");
                }
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZTDPSModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
            ...
            // Check that zTDPS mints (if included) are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 210000 && nHeight > 180000) {
		nSubsidy = 4.4 * COIN;
	}
	else if (nHeight <= 240000 && nHeight > 210000) {
		nSubsidy = 4 * COIN;
	}
	else if (nHeight <= 270000 && nHeight > 240000) {
		nSubsidy = 3.5 * COIN;
	}
	else if (nHeight <= 300000 && nHeight > 270000) {
		nSubsidy = 3 * COIN;
	}
	else if (nHeight <= 390000 && nHeight > 300000) {
		nSubsidy = 2 * COIN;
	}
	else if (nHeight <= 78614448 && nHeight > 390000) {
		nSubsidy = 1 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZTDPSStake)
{
    int64_t ret = 0;
	...
	else if (nHeight <= 80000 && nHeight > Params().LAST_POW_BLOCK()) {
		ret = blockValue * 0.90;  //90%;
	}
	else if (nHeight <= 100000 && nHeight > 80000) {
		ret = blockValue * 0.80;  //80%;
	}
	else if (nHeight <= 78614448 && nHeight > 100000) {
		ret = blockValue * 0.90;  //90%;
	}
	else {
        ret = 0 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from TradePlus_Coin
...
{
    RenameThread("tradeplus_coin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZTDPSMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZTDPS..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZTDPS..."), percent);
        }
...
void RecalculateZTDPSSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZTDPS..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZTDPS..."), percent);
        }

        //Rewrite zTDPS supply
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex));
        ...
        //Remove spends from zTDPS supply
...
bool RecalculateTDPSSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating TDPS supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating TDPS supply..."), percent);
        }
...
{
    // TradePlus_Coin: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZTDPSSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that zTDPS mints are not already known
            if (tx.HasZerocoinMintOutputs()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
    ...
    //Track zTDPS money supply in the block index
    if (!UpdateZTDPSSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zTDPS supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zTdpsSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zTDPS spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zTDPS serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Tradeplus_Coin Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().Tradeplus_CoinBadBlockTime()) && (block.nBits == (uint32_t) Params().Tradeplus_CoinBadBlocknBits())) {
            // accept TradePlus_Coin block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> tdpsInputs;
        std::vector<CTxIn> zTDPSInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zTDPSInputs.push_back(stakeIn);
            }else{
                tdpsInputs.push_back(stakeIn);
            }
        }
        const bool hasTDPSInputs = !tdpsInputs.empty();
        const bool hasZTDPSInputs = !zTDPSInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                if(nHeight >= Params().Zerocoin_StartHeight()) {
                    ...
                    if (isPrivZerocoinSpend || isPublicSpend) {
                        ...
                        if (isPublicSpend) {
                            libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                            PublicCoinSpend publicSpend(params);
                            if (!ZTDPSModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
                                return false;
                            }
                            spend = publicSpend;
                        } else {
                ...
                if(hasTDPSInputs)
                    ...
                    for (const CTxIn& tdpsIn : tdpsInputs){
                        if(tdpsIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : tdpsInputs) {
            ...
            // Now that this loop if completed. Check if we have zTDPS inputs.
            if(hasZTDPSInputs){
                for (const CTxIn& zTdpsInput : zTDPSInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zTdpsInput);
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // TradePlus_Coin: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("ztdps", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("ztdps", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
