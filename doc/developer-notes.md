...
- [Development guidelines](#development-guidelines)
    - [General TradePlus_Coin](#general-tradeplus_coin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
TradePlus_Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play TDPSs (tTDPS)" on the test network, if you
...
TradePlus_Coin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/tradeplus_coin-project/tradeplus_coin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_tradeplus_coin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_tradeplus_coin --log_level=test_suite
$ valgrind -v --leak-check=full src/tradeplus_coind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_tradeplus_coin.coverage/index.html`.
```
...
- BitcoinMiner : Generates TDPSs (if wallet is enabled).
...
However, in open source software such as TradePlus_Coin, where everyone uses
...
pay attention to for reviewers of TradePlus_Coin code.

General TradePlus_Coin
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: TradePlus_Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against TradePlus_Coin so that they can be integrated
...
file descriptors it will cause problems with TradePlus_Coin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:TradePlus_Coin-Project/TradePlus_Coin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `tradeplus_coin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `tradeplus_coin-cli` and the GUI debug console use this table to determine how to
