...
*Setup instructions for a gitian build of TradePlus_Coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the TradePlus_Coin
...
- [Building TradePlus_Coin](#building-tradeplus_coin)
...
Clone the git repositories for tradeplus_coin and gitian and then checkout the tradeplus_coin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/tradeplus_coin-crypto/tradeplus_coin.git
cd tradeplus_coin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the tradeplus_coin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building TradePlus_Coin
----------------

To build TradePlus_Coin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the tradeplus_coin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tradeplus_coin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/tradeplus_coin-crypto/tradeplus_coin
...
For example:
```bash
URL=https://github.com/crowning-/tradeplus_coin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit tradeplus_coin=${COMMIT} --url tradeplus_coin=${URL} ../tradeplus_coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tradeplus_coin=${COMMIT} --url tradeplus_coin=${URL} ../tradeplus_coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tradeplus_coin=${COMMIT} --url tradeplus_coin=${URL} ../tradeplus_coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tradeplus_coin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tradeplus_coin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/tradeplus_coin-build.assert
```
...
[tradeplus_coin/gitian.sigs](https://github.com/tradeplus_coin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
